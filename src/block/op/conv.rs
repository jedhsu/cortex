...
pub trait Glimpse {
    ...
    fn glimpse2() -> Equiv<Conv, Conv> {
        Let(ScalarMul(Conv(s, p, Anone, 𝚇, 𝙮), w) , Conv(s, p, Anone, ScalarMul(𝚇, w), 𝙮));
    }

    fn glimpse3() -> Equiv<Conv, Conv> {
        Let(Conv(s, p, Anone, 𝚇, ElAdd(𝙮, 𝒁)), ElAdd(Conv(s, p, Anone, 𝚇, 𝙮), Conv(s, p, Anone, 𝚇, 𝒁),));
    }

    fn glimpse4() -> Equiv<Conv, Conv> {
        Let(Conv(s, p, Anone, ElAdd(𝚇, 𝙮), 𝒁), (Conv(s, p, Anone, 𝚇, 𝒁), Conv(s, p, Anone, 𝙮, 𝒁)))
    }
